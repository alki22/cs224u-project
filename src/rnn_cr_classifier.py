...
def prepare_train_data(data):
    ...
    non_cr_df = data[data.cr == 0].sample(n=cr_df.shape[0])
...
def simpleRNN(embeddings, hidden_dim=100, dropout=0.1, recurrent_dropout=0.0):
    ...
    x = LSTM(hidden_dim,activation="sigmoid", recurrent_activation="hard_sigmoid", return_sequences=False)(x)
    x = Dropout(dropout)(x)
...
def evaluate(y_true, y_pred, name="tiny"):
    ...
    directory = Path(os.path.join(Config.RUNS_DIR, "cr_classifier", "simple_rnn", name))
...
#        class_weight[c] /= tot
    ...
    joblib.dump(model, Config.RNN_CR_CLASSIFIER_MODEL)
